...
interface StrapiV4Client<T> {
  ...
  create<F = T>(contentType: string, data: Partial<F>, headers?: HeadersInit): Promise<Strapi4ResponseSingle<F>>
  update<F = T>(contentType: string, id: string | number | Partial<F>, data?: Partial<F>, headers?: HeadersInit): Promise<Strapi4ResponseSingle<F>>
  delete<F = T>(contentType: string, id?: string | number, headers?: HeadersInit): Promise<Strapi4ResponseSingle<F>>
}
