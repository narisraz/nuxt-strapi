...
interface StrapiV4Client<T> {
  ...
  create<F = T>(contentType: string, data: Partial<F>, headers?: object): Promise<Strapi4ResponseSingle<F>>
  update<F = T>(contentType: string, id: string | number | Partial<F>, data?: Partial<F>, headers?: object): Promise<Strapi4ResponseSingle<F>>
  delete<F = T>(contentType: string, id?: string | number, headers?: object): Promise<Strapi4ResponseSingle<F>>
}
